...
* CERT\_DIR: The directory where certs (and corresponding signatur request will
  be saved)
...
* CHAIN\_CERT: The path to the Let's encrypt chain certificate (only used for
  stanza-templates)
* CHALLENGE\_FOLDER: The writeable (!) folder in you write challenges and which
...
* hostname: holds the name the certificate will be reqeust for (and what will
  be tested against in the challenge).
* vhost\_file: is used for hinting and is *only read*. It checks the file given
  for the proper cert line (which is autogenerated into the `CERT_DIR`.
...
Add a block like the folloing to config.json (in "ENABLED_DOMAINS", probably
